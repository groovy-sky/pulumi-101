...
## Repo Fundamentals
- YAML-first catalog: `env/` exposes ARM-managed values, `azure/` hosts independent Pulumi projects, and `shared/` stores reusable ARM/Bicep templates.
- Each project keeps its own `Pulumi.<stack>.yaml`; environment drift lives there, never inside `Pulumi.yaml`.
- `shared/containerGroup.json` is the deployed template, with `containerGroup.bicep` maintained manually for readability.
...
- `azure/container-group`: references `env` (`envStackRef`), feeds `azure-native:resources:Deployment` with `shared/containerGroup.json`, and outputs container IDs/FQDN.
...
- When replaying ARM/Bicep, load JSON with `fn::file` + `fn::fromJSON`, then pass `properties.parameters.<param>.value` exactly as shown in `azure/container-group/Pulumi.yaml`.
- Outputs should stay human-friendly (names, IDs, URLs) so downstream services can forward them without transformations.

## Workflows & Commands
- Local flow: `pulumi -C env up`, then `pulumi -C azure/container-group up`, then any additional catalog service. Use `stack select`/`stack init` per environment.
- Containerized flow: `docker build -t pulumi-yaml-aci .` then run `docker run --rm -v $(pwd):/workspace pulumi-yaml-aci -C azure/container-group preview -s dev` (same image works for every project).
...
- Keep `shared/containerGroup.json` and `.bicep` synchronized; the JSON file is what gets deployed.
- The Docker image copies only `env/`, `azure/`, and `shared/`; place new assets under those directories or adjust the `Dockerfile`.
- Use ASCII identifiers/tags and simple interpolations (e.g., `${name}-dns`) to match the ARM template assumptions.

## Extending the Catalog
- Add new services under `azure/<name>` that (1) reference `env`, (2) load their ARM/Bicep template from `shared/`, and (3) expose outputs consumable by other stacks.
