...
Merges global config (services/<provider>/Pulumi.<stack>.yaml) with
...
def map_vars_to_pulumi_config(
    ...
    # Optional grab bag for additional project-scoped config values.
    extra_cfg = vars_.get("projectConfig")
...
def generate_config(
    ...
    if not os.path.exists(global_file):
        raise FileNotFoundError(
            ...
            f"Create it as: services/{provider}/Pulumi.{stack}.yaml"
        )
...
def write_generated_config(
    ...
    header = [
        ...
        f"Config inherited from: services/{provider}/Pulumi.{stack}.yaml",
...
Config resolution:
    1. Global: services/<provider>/Pulumi.<stack>.yaml
